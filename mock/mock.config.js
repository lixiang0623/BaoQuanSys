...
config.call = {
    ...
    rpc: function (opts, callback) {
     ...
     // mock rpcNative 接口  原生自己重写的rpc
     rpcNative: function (opts, callback) {
        var type = opts.url;
        var rpc = require('./rpc/' + type);
        var data = typeof rpc === 'function' ? rpc(opts) : rpc;
        // 防止在业务逻辑中对传入的对象进行了修改
...
config.rpcMockList = [
    // 'test',
    'com.GLMobile.Customer.QryCityInfo',
    'com.GLMobile.Customer.CardListQry',
    'com.GLMobile.Customer.CardListQryCanAdd',
    // 'com.GLMobile.Finance.TransDetailQry',
    'com.GLMobile.O2O.OrderListQry',
